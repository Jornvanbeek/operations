...
for i in range(planes):
    ...
    for j in range(planes):
        if i != j:
            temp.append(0)  # separation time for different runways, aanpassen!
        else:
            temp.append(0)
    s.append(temp)
s = np.array(s)
...
for i in range(planes):
    ...
    # print(thisLHS)
    # a = quicksum(rw[i, k] for k in range(runways))
    ...
    for j in range(planes):  # maybe remove the i part? though this should be fewer constraints overall
        if j != i:
            ...
            for k in range(runways):
                ...
                model.addConstr(lhs=thisLHS, sense=GRB.GREATER_EQUAL, rhs=thisRHS, name="z_[%s,%s]" % (i, j))
